...
import { AddBoardMemberUseCase } from "../../usecases/AddBoardMember";
...
const createBoardMember = (params: { userId: string; role: string }) => ({
  userId: params.userId,
  role: params.role,
  joinedAt: new Date()
});

const createRegularMember = (userId: string) => ({
  userId,
  role: "MEMBER",
  joinedAt: new Date()
});
